...
pub fn decompress(comp_data: []const u8, decompressed_size: u32, alloc: *std.mem.Allocator) ![]u8 {
    var decomp_data = try alloc.alloc(u8, decompressed_size);
    errdefer alloc.free(decomp_data);
    ...
    if (literal_count > decompressed_size or literal_count > comp_data.len)
        return error.Malformed;

    // Copy the initial uncompressed literal
    std.mem.copy(u8, decomp_data[0..literal_count], comp_data[0..literal_count]);
    ...
    while (comp_pos < comp_data.len and decomp_pos <= decompressed_size) {
        ...
        std.mem.copy(u8, decomp_data[decomp_pos..][0..literal_count], comp_data[comp_pos..][0..literal_count]);
        decomp_pos += literal_count;
        comp_pos += literal_count;
    }
